use std::{sync::Arc, fs::{self, File}, io::Write, path::{Path, PathBuf}};

use controller::{SeedPrefix, BasicPrefix, CryptoBox, LocationScheme, Controller, config::ControllerConfig, identifier_controller::IdentifierController, IdentifierPrefix, CesrPrimitive};
use anyhow::Result;
...
fn load_id(alias: String) -> Result<IdentifierController> {
	...
	let registry_id: IdentifierPrefix = fs::read_to_string(registry_path)
		...
		.parse()
		.unwrap();
	let cont = Arc::new(Controller::new(ControllerConfig { db_path: db_path.into(), .. ControllerConfig::default() }).unwrap());
	Ok(IdentifierController::new(identifier, cont, Some(registry_id)))
}

pub async fn incept(priv_key: SeedPrefix, next_key: BasicPrefix, witness: Option<LocationScheme>, alias: String, messagebox: Option<LocationScheme>, watcher: Option<LocationScheme>) -> Result<IdentifierPrefix> {
	let mut store_path = PathBuf::from(".");
	store_path.push(alias);
	let mut db_path = store_path.clone();
	db_path.push("db");
	let cont = Arc::new(Controller::new(ControllerConfig { db_path: db_path.clone().into(), .. ControllerConfig::default() })?);
	let signer = Arc::new(Signer::new_with_seed(&priv_key)?);
	let id = setup_identifier(cont, signer, next_key, witness.unwrap(), messagebox, watcher).await?;
