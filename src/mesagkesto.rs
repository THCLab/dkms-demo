...
use crate::{CliError, utils::{load_identifier, load, load_signer}};
...
pub fn handle_exchange(alias: &str, data: &str, receiver_alias: &str) -> Result<String, CliError> {
	...
	let signature = controller::SelfSigningPrefix::Ed25519Sha512(signer.sign(exn.to_string().as_bytes()).map_err(|_e| MesagkestoError::SigningError)?);
	Ok(signer_id.sign_to_cesr(&exn.to_string(), signature, 0).map_err(|_e| MesagkestoError::SigningError)?)
}
...
pub fn handle_pull(alias: &str) -> Result<String, CliError> {
	...
	let signature = controller::SelfSigningPrefix::Ed25519Sha512(signer.sign(qry.to_string().as_bytes()).map_err(|_e| MesagkestoError::SigningError)?);
	Ok(signer_id.sign_to_cesr(&qry.to_string(), signature, 0).map_err(|_e| MesagkestoError::SigningError)?)
}
