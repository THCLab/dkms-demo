...
MAILBOX="$(curl -s -X POST "$WITNESS_ADDRESS"/query -d $(cat "$INPUT_DATA_DIR"/identifier1/mailbox_qry_0))"
...
MESSAGEBOX_RESPONSE=$(curl -s -X POST $MESAGKESTO_ADDRESS -d $(cat "$INPUT_DATA_DIR"/messagebox/qry))
# echo "mesagebox response: \n"
# echo $MESSAGEBOX_RESPONSE

# Verify gotten acdc
TEL=$(curl -s -X POST "$WITNESS_ADDRESS"/query/tel -d $(cat "$INPUT_DATA_DIR"/messagebox/tel_qry))
